...
|---|---|---|---|
| FR-001 | Создание/открытие/сохранение проекта | desktop: `VMTSerializer` | «New project» открывает диалог: **имя проекта**, **имя корневой машины**, **алфабит** (≥2 символа, λ первым) — все обязательны. Open/Save — как раньше. |
| FR-002 | Редактирование алфавита | `formalphabit.ui` | Алфавит задаётся при создании проекта; первый символ = λ; для узла **Complex** при размещении обязательно **имя подмашины**. |
| FR-003 | Визуальный редактор диаграммы | `FormEditorMachine` | Канвас на всё поле; inspector — оверлей. При drag узла (pointer): двигаются прямоугольник с подписью и все связанные связи (live preview до mouseup). Pan — сдвиг вида. После mouseup — snap к сетке и commit. |
...
| FR-006 | Связи (переходы) с условиями | `VMTTransitionImpl`, `VMTTransitionConditions` | Инструмент link: клик на исходный узел → live-preview линии → клик на целевой узел; маршрут A\*. При выделении связи — мультивыбор символов алфавита, метка `*` при полном покрытии; подпись перетаскивается вдоль линии. |
...
| FR-016 | Экспорт в «четвёрки» | `VMTExport4th` | Экран Export 4th: `exportFourth()`, настройки формата, вывод в textarea, копирование в буфер; рекурсия complex — ошибка; left/right/write/complex (inner) — как десктоп; word/copy — упрощённо без полного инлайна. |
